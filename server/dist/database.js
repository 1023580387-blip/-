...
function seedData() {
    ...
    const insertLang = exports.db.prepare('INSERT OR IGNORE INTO languages (id, name, name_native, flag) VALUES (?, ?, ?, ?)');
    ...
    const insertAchievement = exports.db.prepare('INSERT OR IGNORE INTO achievements (name, description, icon, condition_type, condition_value, reward_xp) VALUES (?, ?, ?, ?, ?, ?)');
...
function insertCourse(languageId, level, title, description, thumbnail, orderIndex) {
    const existing = exports.db.prepare('SELECT id FROM courses WHERE language_id = ? AND title = ?').get(languageId, title);
    if (existing)
        return existing.id;
    const result = exports.db.prepare('INSERT INTO courses (language_id, level, title, description, thumbnail, order_index) VALUES (?, ?, ?, ?, ?, ?)').run(languageId, level, title, description, thumbnail, orderIndex);
    return Number(result.lastInsertRowid);
}
function insertUnit(courseId, title, description, orderIndex) {
    const existing = exports.db.prepare('SELECT id FROM units WHERE course_id = ? AND title = ?').get(courseId, title);
    if (existing)
        return existing.id;
    const result = exports.db.prepare('INSERT INTO units (course_id, title, description, order_index) VALUES (?, ?, ?, ?)').run(courseId, title, description, orderIndex);
    return Number(result.lastInsertRowid);
}
function insertWord(unitId, wordData) {
    const existing = exports.db.prepare('SELECT id FROM words WHERE unit_id = ? AND word = ?').get(unitId, wordData.word);
    if (existing)
        return existing.id;
    const result = exports.db.prepare('INSERT INTO words (unit_id, word, translation, pronunciation, audio_url, example, example_translation, part_of_speech, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)').run(unitId, wordData.word, wordData.translation, wordData.pronunciation || '', '', wordData.example || '', wordData.example_translation || '', wordData.part_of_speech || '', wordData.difficulty || 1);
    return Number(result.lastInsertRowid);
}
function seedUnit1Words(unitId) {
    const words = [
...
function seedUnit2Words(unitId) {
    const words = [
...
function seedRestaurantWords(unitId) {
    const words = [
...
function seedUnit1Grammar(unitId) {
    const exercises = [
        {
    ...
    exercises.forEach(e => {
        const existing = exports.db.prepare('SELECT id FROM grammar_exercises WHERE unit_id = ? AND question = ?').get(unitId, e.question);
        if (!existing) {
            exports.db.prepare('INSERT INTO grammar_exercises (unit_id, grammar_rule, explanation, question, options, correct_answer, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?)').run(unitId, e.grammar_rule, e.explanation, e.question, e.options, e.correct_answer, e.difficulty);
        }
    });
}
function seedUnit1Listening(unitId) {
    const exercises = [
        {
    ...
    exercises.forEach(e => {
        const existing = exports.db.prepare('SELECT id FROM listening_exercises WHERE unit_id = ? AND script = ?').get(unitId, e.script);
        if (!existing) {
            exports.db.prepare('INSERT INTO listening_exercises (unit_id, script, translation, question, options, correct_answer, difficulty) VALUES (?, ?, ?, ?, ?, ?, ?)').run(unitId, e.script, e.translation, e.question, e.options, e.correct_answer, e.difficulty);
        }
    });
}
function seedUnit1Speaking(unitId) {
    const phrases = [
    ...
    phrases.forEach(p => {
        const existing = exports.db.prepare('SELECT id FROM speaking_exercises WHERE unit_id = ? AND phrase = ?').get(unitId, p.phrase);
        if (!existing) {
            exports.db.prepare('INSERT INTO speaking_exercises (unit_id, phrase, translation, pronunciation_hint, audio_url, difficulty) VALUES (?, ?, ?, ?, ?, ?)').run(unitId, p.phrase, p.translation, p.pronunciation_hint, '', p.difficulty);
        }
    });
}
