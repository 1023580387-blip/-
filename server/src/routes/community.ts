...
router.get('/', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const { languageId, category, page = 1, limit = 20 } = req.query as any;
...
// 获取热门帖子
router.get('/hot', authMiddleware, (_req, res: Response) => {
  try {
    const posts = db.prepare(`
      SELECT p.*, 
        u.username, 
...
// 获取帖子详情
router.get('/:postId', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
...
router.post('/', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    // 检查社区成就
    const postCount = db.prepare('SELECT COUNT(*) as count FROM posts WHERE user_id = ?').get(userId) as any;
    const achievements = checkAchievements(userId, 'posts', postCount.count || 0);
...
// 点赞帖子
router.post('/:postId/like', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const { postId } = req.params;
...
// 创建评论
router.post('/:postId/comments', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const { postId } = req.params;
...
// 点赞评论
router.post('/comments/:commentId/like', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
