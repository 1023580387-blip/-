...
// 获取某语言的所有课程
router.get('/courses/:languageId', (req, res: Response) => {
  try {
...
// 获取课程详情（含单元）
router.get('/courses/detail/:courseId', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    // 获取用户的学习进度
    const unitsWithProgress = (units as any[]).map(unit => {
      const progress = db.prepare(`
        SELECT status, score 
        FROM user_progress 
        WHERE user_id = ? AND unit_id = ? AND exercise_type = 'unit'
      `).get(userId, unit.id) as any;
...
// 获取单元详情（含所有练习）
router.get('/units/:unitId', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    const words = db.prepare('SELECT * FROM words WHERE unit_id = ? ORDER BY id ASC').all(unitId);
    const grammarExercises = db.prepare('SELECT * FROM grammar_exercises WHERE unit_id = ? ORDER BY id ASC').all(unitId);
    const listeningExercises = db.prepare('SELECT * FROM listening_exercises WHERE unit_id = ? ORDER BY id ASC').all(unitId);
    const speakingExercises = db.prepare('SELECT * FROM speaking_exercises WHERE unit_id = ? ORDER BY id ASC').all(unitId);

    // 获取单词熟练度
    const wordsWithMastery = (words as any[]).map(word => {
      const mastery = db.prepare(`
        SELECT mastery_level, correct_count, wrong_count, next_review_at
        FROM user_word_mastery
        WHERE user_id = ? AND word_id = ?
      `).get(userId, word.id) as any;
...
router.get('/recommendations', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    const user = db.prepare('SELECT target_language, proficiency_level, experience FROM users WHERE id = ?').get(userId) as any;
    ...
    for (const course of courses as any[]) {
      const units = db.prepare('SELECT * FROM units WHERE course_id = ? ORDER BY order_index ASC').all(course.id) as any[];
