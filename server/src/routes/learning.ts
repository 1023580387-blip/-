...
// 检查并解锁成就
export function checkAchievements(userId: number, type: string, count: number): any[] {
  const unlocked: any[] = [];
...
// 添加经验值
function addXP(userId: number, xp: number): { leveledUp: boolean; newLevel: number } {
  const user = db.prepare('SELECT level, experience FROM users WHERE id = ?').get(userId) as any;
  ...
  db.prepare('UPDATE users SET experience = ?, level = ? WHERE id = ?').run(newXP, newLevel, userId);
...
// 更新每日学习记录
function updateDailyRecord(userId: number, data: { xp?: number; words?: number; minutes?: number; exercises?: number }) {
  const today = new Date().toISOString().split('T')[0];
  
  const existing = db.prepare('SELECT * FROM daily_records WHERE user_id = ? AND study_date = ?').get(userId, today) as any;
...
// 更新或插入学习进度
function upsertProgress(userId: number, data: {
  ...
  const existing = db.prepare(`
    ...
    AND exercise_type = ?
  `).get(userId, data.courseId || null, data.unitId || null, data.wordId || null, data.exerciseType) as any;
...
// 记录单词学习结果
router.post('/word-result', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    // 更新单词熟练度
    const existing = db.prepare('SELECT * FROM user_word_mastery WHERE user_id = ? AND word_id = ?').get(userId, wordId) as any;
...
// 记录练习结果（语法/听力/口语）
router.post('/exercise-result', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    ...
    `).get(userId, exerciseType) as any;
    ...
    // 检查连续学习天数成就
    const user = db.prepare('SELECT streak_days FROM users WHERE id = ?').get(userId) as any;
    if (user) {
...
// 完成单元
router.post('/complete-unit', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    const userId = req.user!.id;
    ...
    if (user.last_study_date !== today) {
      ...
      db.prepare('UPDATE users SET last_study_date = ?, streak_days = ? WHERE id = ?').run(today, newStreak, userId);
...
router.get('/stats', authMiddleware, (req: AuthRequest, res: Response) => {
  try {
    ...
    const user = db.prepare('SELECT level, experience, streak_days, daily_goal FROM users WHERE id = ?').get(userId) as any;
    if (!user) {
    ...
    // 今日进度
    const today = new Date().toISOString().split('T')[0];
    const todayRecord = db.prepare('SELECT * FROM daily_records WHERE user_id = ? AND study_date = ?').get(userId, today) as any;
